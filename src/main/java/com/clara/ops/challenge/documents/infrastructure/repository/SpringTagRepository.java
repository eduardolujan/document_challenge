...
public class SpringTagRepository implements TagRepository {
  ...
  public SpringTagRepository(
      JpaTagRepository jpaTagRepository, EntityModelMapper entityModelMapper) {
  ...
  public Optional<Tag> findById(String name) {
    return jpaTagRepository
        .findByName(name)
        .map(tagEntity -> entityModelMapper.getModelMapper().map(tagEntity, Tag.class));
  }
