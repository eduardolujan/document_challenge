...
public class SpringDocumentRepository implements DocumentRepository {
  ...
  public SpringDocumentRepository(JpaDocumentRepository jpaDocumentRepository, EntityModelMapper entityModelMapper) {
  ...
  public List<Document> search(
    ...
    return data.stream().map(documentEntity -> entityModelMapper.getModelMapper().map(documentEntity, Document.class)).collect(Collectors.toList());
  ...
  public Document save(Document document) {
    jpaDocumentRepository.save(entityModelMapper.getModelMapper().map(document, DocumentEntity.class));
    return document;
  }

  @Override
  public Optional<Document> findById(UUID id) {
    return jpaDocumentRepository.findById(id).map(documentEntity -> entityModelMapper.getModelMapper().map(documentEntity, Document.class));
  }
